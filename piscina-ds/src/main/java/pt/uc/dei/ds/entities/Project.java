...
@NamedQueries({ @NamedQuery(name="Project.findAll", query="SELECT p FROM Project p"),
	...
	@NamedQuery(name="Project.findActualCost", query="SELECT SUM(tw.hoursWorked*t.hourCost) from Taskwork tw join tw.allocation a JOIN a.task t join t.project p where p.id=:id"),
	@NamedQuery(name="Project.findProjectCostInDate", query="SELECT SUM(tw.hoursWorked*t.hourCost) from Taskwork tw join tw.allocation a JOIN a.task t join t.project p where p.id=:id AND tw.creationDate<=:date"),
